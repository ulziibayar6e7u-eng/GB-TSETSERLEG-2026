...
import MonthlyEditor, { type MonthlyData } from '@/components/MonthlyEditor'
...
export default function MonthlyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function load() {
    ...
    if (p) {
      const pp = p as unknown as Plan
      setData({
        theme: pp.theme || '',
        method: pp.method || '',
        goals: pp.goals || '',
        week_themes: pp.week_themes && pp.week_themes.length ? pp.week_themes : [{week:1,theme:''},{week:2,theme:''},{week:3,theme:''},{week:4,theme:''}],
  ...
  async function approve(level: 'excellent' | 'good' | 'revise') {
    if (!me || !plan) return
    if (level === 'revise' && !reviewNote.trim()) { alert('Засах шалтгаанаа бичнэ үү'); return }
    setSaving(true)
    await supabase.from('monthly_plans').update({
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 mb-4">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div>
              ...
              <h1 className="text-xl font-bold text-slate-800">{plan.year} · {MONTHS[plan.month - 1]} · {plan.theme || '(сэдэвгүй)'}</h1>
            ...
            {(isOwner || me?.is_admin) && (
              <button onClick={remove} className="text-red-600 hover:text-red-800 text-sm font-medium">Устгах</button>
            )}
          </div>
          {plan.approver_note && (
        ...
        <MonthlyEditor value={data} onChange={setData} readOnly={readOnly} />
        ...
        <div className="mt-4 bg-white rounded-2xl border border-slate-200 p-4 sticky bottom-0 z-10">
          {isOwner && (plan.status === 'draft' || plan.status === 'returned') && (
            <div className="flex gap-2 justify-end">
              <button onClick={() => save(false)} disabled={saving} className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg font-medium disabled:opacity-50">💾 Хадгалах</button>
              <button onClick={() => save(true)} disabled={saving} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium disabled:opacity-50">📤 Батлуулахаар илгээх</button>
            </div>
          )}
          {canApprove && plan.status === 'submitted' && (
            <div className="space-y-3">
              <div>
                ...
                <textarea rows={2} value={reviewNote} onChange={(e) => setReviewNote(e.target.value)} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div className="flex gap-2 justify-end flex-wrap">
                {LEVELS.map((l) => (
                  <button key={l.value} onClick={() => approve(l.value)} disabled={saving} className={`px-4 py-2 rounded-lg font-medium disabled:opacity-50 ${l.color}`}>{l.label}</button>
                ))}
              </div>
            </div>
          )}
          {plan.status === 'approved' && (
            <div className="text-center text-sm text-emerald-700">✅ Батлагдсан</div>
          )}
        </div>
      </div>
    </div>
